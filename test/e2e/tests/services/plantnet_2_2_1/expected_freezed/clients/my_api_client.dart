...
import '../models/model30.dart';
import '../models/model32.dart';
...
abstract class MyApiClient {
  ...
  @GET('/v2/languages')
  Future<LanguagesList> getV2Languages({@Query('api-key') String? apiKey});
  ...
  /// [apiKey] - Your private API key.
  @GET('/v2/quota')
  Future<Model2> getV2Quota({@Query('api-key') String? apiKey});
  ...
  /// [type] - Projects type.
  ///
  /// [pageSize] - Number of results per page. Must be used along with "page". Set to "" (empty string) to disable pagination.
  ///
  /// [page] - Page number starting at 1. Must be used along with "pageSize". Set to "" (empty string) to disable pagination.
  ///
  ...
  Future<SpeciesList> getV2Species({
    ...
    @Query('lang') Lang? lang = Lang.en,
    @Query('type') Type? type = Type.kt,
    @Query('pageSize') String? pageSize = '100',
  ...
  /// [apiKey] - Your private API key.
  @GET('/v2/subscription')
  Future<Model4> getV2Subscription({@Query('api-key') String? apiKey});
  ...
  ///
  /// [detailed] - If true, results other than species predictions (family, genus…) will be added (slower).
  ///
  /// [apiKey] - Your private API key.
  ///
  ...
  Future<IdentificationResult> getV2IdentifyProject({
    ...
    @Query('lang') Lang? lang = Lang.en,
    @Query('detailed') bool? detailed = false,
  ...
  /// Species name alignment by project.
  ///
  /// Aligns a species name on a specific Pl@ntNet taxonomic referential.
  ///
  /// [name] - Species name to align.
  ///
  /// [authorship] - Set to true when species name to align contains authorship.
  ///
  /// [synonyms] - Set to true to look for synonyms as well as accepted names.
  ///
  /// [fuzzy] - Set to true to perform a fuzzy search.
  ///
  /// [apiKey] - Your private API key.
  @GET('/v2/projects/{id}/species/align')
  Future<String> getV2ProjectsIdSpeciesAlign({
    @Query('name') required String name,
    @Query('api-key') String? apiKey,
    @Query('authorship') bool? authorship = true,
    @Query('synonyms') bool? synonyms = false,
    @Query('fuzzy') bool? fuzzy = false,
  });
}
