...
abstract class UsersClient {
  ...
  @POST('/users')
  Future<HttpResponse<User>> createUser({
  ...
  /// [userId] - User ID.
  @GET('/users/{userId}')
  Future<HttpResponse<User>> getUser({@Path('userId') required String userId});
  ...
  /// [userId] - User ID.
  @PATCH('/users/{userId}')
  Future<HttpResponse<User>> patchUser({
  ...
  Future<HttpResponse<FileUploadResponse>> uploadAvatar({
