...
void main() {
  ...
  final edgeCases = [
    ...
    // Dots
    (text: 'com.example.api', expectedWords: ['com', 'example', 'api']),
    (text: 'v2.0.1', expectedWords: ['v2', '0', '1']),
    // Forward slash
    (text: 'users/profile', expectedWords: ['users', 'profile']),
    (text: 'api/v1/users', expectedWords: ['api', 'v1', 'users']),
    // At symbol
    (text: 'user@domain', expectedWords: ['user', 'domain']),
    (text: '@deprecated_method', expectedWords: ['deprecated', 'method']),
    // Curly braces
    (text: '{userId}', expectedWords: ['user', 'id']),
    (text: 'template{param}', expectedWords: ['template', 'param']),
    // Parentheses
    (text: 'method()', expectedWords: ['method']),
    (text: 'calc(value)', expectedWords: ['calc', 'value']),
    // Square brackets
    (text: 'array[index]', expectedWords: ['array', 'index']),
    (text: 'items[0]', expectedWords: ['items', '0']),
    // Angle brackets
    (text: 'List<String>', expectedWords: ['list', 'string']),
    (text: '<T>generic', expectedWords: ['t', 'generic']),
    // Colon
    (text: 'namespace:method', expectedWords: ['namespace', 'method']),
    (text: 'http://example.com', expectedWords: ['http', 'example', 'com']),
    // Semicolon
    (text: 'part1;part2', expectedWords: ['part1', 'part2']),
    // Backtick
    (text: '`literal`', expectedWords: ['literal']),
    (text: 'template`string`', expectedWords: ['template', 'string']),
    // Tilde
    (text: '~deprecated', expectedWords: ['deprecated']),
    (text: 'version~1.0', expectedWords: ['version', '1', '0']),
    // Exclamation
    (text: '!important', expectedWords: ['important']),
    (text: 'not!equal', expectedWords: ['not', 'equal']),
    // Dollar sign
    (text: r'$variable', expectedWords: ['variable']),
    (text: r'price$USD', expectedWords: ['price', 'usd']),
    // Percent
    (text: '100%complete', expectedWords: ['100', 'complete']),
    (text: 'progress%', expectedWords: ['progress']),
    // Caret
    (text: '^version', expectedWords: ['version']),
    (text: 'base^exponent', expectedWords: ['base', 'exponent']),
    // Ampersand
    (text: 'fish&chips', expectedWords: ['fish', 'chips']),
    (text: '&reference', expectedWords: ['reference']),
    // Asterisk
    (text: '*pointer', expectedWords: ['pointer']),
    (text: 'wild*card', expectedWords: ['wild', 'card']),
    // Plus
    (text: 'C++', expectedWords: ['c']),
    (text: 'item+added', expectedWords: ['item', 'added']),
    // Equals
    (text: 'key=value', expectedWords: ['key', 'value']),
    ...
    (text: '5иллегалчарактер', expectedWords: ['5иллегалчарактер']),
    (text: 'café_data', expectedWords: ['café', 'data']),
    (text: 'naïve_user', expectedWords: ['naïve', 'user']),
    (text: 'helloКир', expectedWords: ['helloкир']),
    (text: 'データベース_test', expectedWords: ['データベース', 'test']),
    (text: 'αβγ_values', expectedWords: ['αβγ', 'values']),
    (text: '测试_method', expectedWords: ['测试', 'method']),
    (text: 'émoji_😀_test', expectedWords: ['émoji', '😀', 'test']),
    (text: 'ñoño_field', expectedWords: ['ñoño', 'field']),
    (text: 'résumé', expectedWords: ['résumé']),
    (text: 'straße_name', expectedWords: ['straße', 'name']),
    (text: 'москва123', expectedWords: ['москва123']),
    (text: 'π_value', expectedWords: ['π', 'value']),
    (text: 'caféAPI', expectedWords: ['café', 'api']),
  ];
